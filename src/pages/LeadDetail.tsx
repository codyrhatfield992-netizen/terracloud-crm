        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <StatusBadge variant={priorityVariant(priority)}>{getPriorityLabel(priority)}</StatusBadge>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            <div className="space-y-3">
              ...
              <Row label={<><DollarSign className="h-3.5 w-3.5" />Est. Value</>}>
                {formatCurrency(Number(lead.estimated_value))}
              </Row>
              <Row label={<><Calendar className="h-3.5 w-3.5" />Follow-Up</>}>
              ...
              {lead.tags?.length > 0 && (
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-wrap gap-1.5 justify-end">
                    {lead.tags.map((t) => (
                      <span key={t} className="px-2 py-0.5 rounded bg-secondary text-xs font-medium text-foreground">{t}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            {property && (
              <div className="bg-card border border-border rounded-lg p-6">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {property.city}, {property.state} · ARV: {formatCurrency(Number(property.arv))}
                </p>
              </div>
            )}
          </div>
...
function Row({ label, children }: { label: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-sm text-muted-foreground flex items-center gap-1.5">{label}</span>
