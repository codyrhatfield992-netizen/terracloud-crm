...
import { LayoutGrid, List, Plus, Search, X, ChevronDown, ChevronUp, MapPin, Calendar } from "lucide-react";
...
import { PIPELINE_STAGES, getStageLabel, formatCurrency, type StageId } from "@/lib/constants";
...
function KanbanCard({ lead, onDragStart, contactName, propertyAddress }: { lead: DbLead; onDragStart: (e: React.DragEvent, id: string) => void; contactName?: string; propertyAddress?: string }) {
  return (
    <Link to={`/leads/${lead.id}`} draggable onDragStart={(e) => { e.stopPropagation(); onDragStart(e, lead.id); }}
      ...
      {propertyAddress && (
        <div className="flex items-center gap-1.5 mt-0.5"><MapPin className="h-3 w-3 text-muted-foreground shrink-0" /><p className="text-xs text-muted-foreground truncate">{propertyAddress}</p></div>
      )}
      {lead.tags.length > 0 && (
        <div className="flex items-center gap-1 mt-2 flex-wrap">
          {lead.tags.slice(0, 2).map(t => <StatusBadge key={t} variant={t === "Hot" || t === "Urgent" ? "destructive" : "outline"}>{t}</StatusBadge>)}
        </div>
      )}
      <div className="flex items-center justify-between mt-3 pt-2 border-t border-border">
        ...
        {lead.next_follow_up ? (
          <StatusBadge variant={getFollowUpVariant(lead.next_follow_up)} className="text-[10px]">
            <Calendar className="h-2.5 w-2.5 mr-1" />{lead.next_follow_up}
          </StatusBadge>
...
function NewLeadModal({ open, onClose, onSubmit }: { open: boolean; onClose: () => void; onSubmit: (l: Partial<DbLead>) => void }) {
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ title: title.trim(), source, priority, estimated_value: parseInt(estimatedValue) || 0, next_follow_up: nextFollowUp || null, tags: [], stage: "new" });
    onClose();
    setTitle(""); setSource("Website"); setPriority("Medium"); setEstimatedValue(""); setNextFollowUp("");
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-card border border-border rounded-lg w-full max-w-[560px] mx-4 shadow-2xl animate-scale-in">
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input value={title} onChange={e => { setTitle(e.target.value); setErrors({}); }}
              ...
              className={`w-full h-9 px-3 rounded-md bg-secondary border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary ${errors.title ? "border-destructive" : "border-border"}`} autoFocus />
            {errors.title && <p className="text-xs text-destructive mt-1">{errors.title}</p>}
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-muted-foreground block mb-1">Source</label>
              <select value={source} onChange={e => setSource(e.target.value)} className="w-full h-9 px-3 rounded-md bg-secondary border border-border text-sm text-foreground focus:outline-none focus:border-primary">
                {["Website", "Referral", "Cold Call", "Direct Mail", "Facebook Ads", "Google Ads", "Zillow", "Other"].map(s => <option key={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground block mb-1">Priority</label>
              <select value={priority} onChange={e => setPriority(e.target.value)} className="w-full h-9 px-3 rounded-md bg-secondary border border-border text-sm text-foreground focus:outline-none focus:border-primary">
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-border">
          ...
          <button onClick={handleSubmit} className="h-9 px-5 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90">Create Lead</button>
        </div>
      </div>
    </div>
  );
...
export default function Leads() {
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter(l => l.title.toLowerCase().includes(q) || (l.contact_id && contactMap[l.contact_id]?.toLowerCase().includes(q)));
  }, [leads, search, contactMap]);
  ...
  const handleStageDrop = useCallback((leadId: string, newStage: string) => {
  ...
  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">{filtered.length} lead{filtered.length !== 1 ? "s" : ""}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center border border-border rounded-md overflow-hidden">
              <button onClick={() => setView("kanban")} className={`h-9 w-9 flex items-center justify-center transition-colors ${view === "kanban" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}><LayoutGrid className="h-4 w-4" /></button>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6 max-w-sm">Create your first lead to start building your pipeline</p>
            <button onClick={() => setShowModal(true)} className="h-9 px-4 flex items-center gap-2 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors">
              <Plus className="h-4 w-4" /> New Lead
            </button>
          </div>
            ...
            {PIPELINE_STAGES.map(stage => {
              ...
              return (
                <div key={stage.id}
                  ...
                  onDrop={e => { e.preventDefault(); const id = e.dataTransfer.getData("text/plain"); if (id) handleStageDrop(id, stage.id); setDragOverStage(null); }}>
                ...
                <tbody>
                  {filtered.map((lead, i) => (
                    <tr key={lead.id} onClick={() => navigate(`/leads/${lead.id}`)}
                      ...
                      <td className="px-4 py-3 text-sm text-muted-foreground">{new Date(lead.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <NewLeadModal open={showModal} onClose={() => setShowModal(false)} onSubmit={l => createLead.mutate(l)} />
    </AppLayout>
  );
}
