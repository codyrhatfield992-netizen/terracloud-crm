...
export default function PracticeDashboard() {
  ...
  return <AppLayout><div className="space-y-6 max-w-[1680px]">
    <PageHeader eyebrow="Learner Operating Console" title="Practice Dashboard" description="Track readiness, recommended drills, leaderboard pressure, and sales simulation performance in one command view." action={<Link to="/practice/launch" className="h-11 px-5 rounded-md xr-button-primary text-sm font-semibold inline-flex items-center gap-2"><Play className="h-4 w-4" /> Launch Practice</Link>} />
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
      ...
      <div className="xl:col-span-4 space-y-5">
        <div className="xr-panel-strong rounded-xl p-5 flex items-center gap-5"><ScoreRing value={data.lastSession.score} label="Prestige Score" /><div><p className="text-sm font-semibold">Next drill</p><p className="text-sm text-muted-foreground mt-1">{data.recommendedScenario.summary}</p><Link to="/practice/launch" className="mt-4 inline-flex items-center gap-1.5 text-sm text-primary hover:text-xr-amber transition">Configure drill <ArrowUpRight className="h-3.5 w-3.5" /></Link></div></div>
        <div className="xr-panel rounded-xl p-5"><h2 className="text-sm font-semibold mb-3 flex items-center gap-2"><Trophy className="h-4 w-4 text-warning" /> Leaderboard Preview</h2>{data.leaderboardPreview.map((e) => <div key={e.id} className="flex items-center justify-between py-3 border-b border-border last:border-0"><div><p className="text-sm text-foreground">#{e.rank} {e.learnerName}</p><p className="text-xs text-muted-foreground">{e.team}</p></div><span className="font-mono text-primary">{e.score}</span></div>)}</div>
      </div>
    </div>
  </div></AppLayout>;
}
