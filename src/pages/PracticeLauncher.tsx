...
function Selector<T extends string>({ title, values, value, onChange }: { title: string; values: T[]; value: T; onChange: (v: T) => void }) { return <div className="xr-glass rounded-xl p-5"><p className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground mb-3">{title}</p><div className="grid grid-cols-2 md:grid-cols-3 gap-3">{values.map((v) => <button key={v} onClick={() => onChange(v)} className={`rounded-lg border border-border px-3 py-3 text-sm capitalize text-left transition ${value === v ? "bg-primary text-primary-foreground" : "bg-card hover:border-primary/40 text-foreground"}`}>{v.replaceAll("_", " ")}</button>)}</div></div>; }
