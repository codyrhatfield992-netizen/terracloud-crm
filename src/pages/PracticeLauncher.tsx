  ...
  return <AppLayout><div className="space-y-6 max-w-[1500px]"><PageHeader eyebrow="Practice Launcher" title="Build a Simulation" description="Compose a text-first AI practice session from generic scenarios, CRM leads, contacts, or listings. Voice and hybrid modes are scaffolded for realtime expansion." />
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
      <div className="xl:col-span-7 space-y-5">
        <Selector title="Drill type" values={drills} value={drillType} onChange={(value) => setDrillType(value)} />
        <Selector title="Scenario source" values={sources} value={source} onChange={(value) => setSource(value)} />
        ...
        <Selector title="Difficulty" values={difficulties} value={difficulty} onChange={(value) => setDifficulty(value)} />
      </div>
...
function Selector<T extends string>({ title, values, value, onChange }: { title: string; values: readonly T[]; value: T; onChange: (value: T) => void }) { return <div className="xr-glass rounded-xl p-5"><p className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground mb-3">{title}</p><div className="grid grid-cols-2 md:grid-cols-3 gap-3">{values.map((v) => <button key={v} onClick={() => onChange(v)} className={`rounded-lg border border-border px-3 py-3 text-sm capitalize text-left transition ${value === v ? "bg-primary text-primary-foreground" : "bg-card hover:border-primary/40 text-foreground"}`}>{v.replace(/_/g, " ")}</button>)}</div></div>; }
