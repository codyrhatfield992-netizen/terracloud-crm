...
import { formatDate, getContactTypeLabel, normalizeContactType } from "@/lib/constants";
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              <StatusBadge variant={contactTypeVariant(type)}>{getContactTypeLabel(type)}</StatusBadge>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3"><Mail className="h-4 w-4 text-muted-foreground shrink-0" /><span className="text-sm text-foreground">{contact.email || "No email"}</span></div>
              <div className="flex items-center gap-3"><Phone className="h-4 w-4 text-muted-foreground shrink-0" /><span className="text-sm text-foreground">{contact.phone || "No phone"}</span></div>
              {contact.source && (
                <div className="flex items-center gap-3"><span className="text-sm text-muted-foreground">Source:</span><span className="text-sm text-foreground">{contact.source}</span></div>
              )}
              <div className="flex items-center gap-3"><Calendar className="h-4 w-4 text-muted-foreground shrink-0" /><span className="text-sm text-muted-foreground">Created {formatDate(contact.created_at)}</span></div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-card border border-border rounded-lg p-6 max-w-md w-full mx-4 shadow-2xl animate-scale-in">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => deleteContact.mutate(contact.id, { onSuccess: () => navigate("/contacts") })} disabled={deleteContact.isPending}
